...
export async function WhyUs() {
  ...
  return (
    <section id={t("id")} className="scroll-mt-28 py-24">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <RevealOnScroll>
          ...
          <p className="mt-4 max-w-3xl text-lg leading-relaxed text-jac-silver-300">
            {t("intro2")}
          </p>
        </RevealOnScroll>

        <RevealOnScroll delayMs={60}>
          <h3 className="mt-12 text-sm font-semibold uppercase tracking-[0.18em] text-jac-blue-bright md:mt-14">
            {t("deliverHeading")}
          </h3>
        </RevealOnScroll>

        <ul className="mt-8 grid gap-5 sm:grid-cols-2 lg:gap-8">
          {items.map((item, i) => (
            <li key={`${i}-${item.slice(0, 24)}`} className="min-w-0">
              <RevealOnScroll
                delayMs={i * 75}
